...
function Registration() {
  ...
  const [checkedListForStatus, setCheckedListForStatus] = useState(
    uncheckAll(radioOptionsForStatus)
  );
  ...
  const onSubmit = (data) => {
    if (data && data !== "") {
      ...
      data.files = fileDownloadURL;
      console.log(fileDownloadURL);
      addDoc(collection(db, "beforeRegistration"), {
        json: data,
      })
        .then(() => {
  ...
  return (
    <div>
      ...
      <div className="flex flex-col-2 sm:flex-col-2 h-fit  w-full justify-center">
        <div>
          <form
            className=" bg-gray-500 py-5 px-12 mt-40 w-auto rounded-lg"
            onSubmit={handleSubmit(onSubmit)}
          >
            <h2 className="text-4xl text-white font-bold  mb-2  text-center">
              REGISTRATION FOR
              <br /> PARTICIPATING EVENT
            </h2>
            ...
            <div className="flex flex-row text-black-100  py-4">
              <label>Year</label>
              <select
                className="text-black w-1/4 ml-10"
            ...
            <div className="flex flex-col text-black-100 py-2">
              <label>
                Organizer Details
                <br />
                (college/company name and address)
              </label>
            ...
            <div className="flex flex-col text-black-100 py-2">
              ...
              <input
                {...register("noOfEvent", {
                  required: "Number of Event is required",
                })}
                placeholder="noOfEvent"
              />
              {errors.noOfEvent && (
                <p class="error-text" role="alert">
                  {errors.noOfEvent?.message}
                </p>
              )}
            </div>
            <div className="flex flex-col text-left   text-black-100 py-2">
              ...
              <input
                {...register("nameOfEvent", {
                  required: "Name of Events is required",
                })}
                placeholder="nameOfEvent"
              />
              {errors.nameOfEvent && (
                <p class="error-text" role="alert">
                  {errors.nameOfEvent?.message}
                </p>
              )}
            </div>
            ...
            <div className="text-center pt-3 ">
              <button
                ...
                className="btn font-bold bg-gray-700 text-white mt-4 btn-default p-2 "
              >
                RESET
              </button>
              <button
                type="submit"
                className="btn font-bold bg-gray-700 text-white  mt-4 btn-default p-2  ml-16"
              >
                SUBMIT
              </button>
            </div>
